...
class Find:
    ...
    def start_change(self):
        ...
        parent = None
        while self.change(word, parent) == False:
            new_word = self.queue.get()
            print new_word
            word = new_word.items()[0][0]
            parent = new_word.items()[0][1]

    def change(self, word, parent = None):
        unchanged = word
        word = list(word)
        new = None
        for letter in range(len(word)):
            for l in string.ascii_lowercase:
                ...
                if self.search(new, self.dictionary) == True and new != unchanged and new != parent and self.search(new, self.bad_words) == False:
                    self.queue.put({new:unchanged})
